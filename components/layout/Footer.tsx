                ...
                <div className="flex flex-wrap xl:flex-nowrap items-center gap-x-3 gap-y-1.5 text-[13px] text-white/75 min-w-0">
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <div className="flex items-center">
                      {loc.phone.split(',').map((p, i, arr) => (
                        <span key={i}>
                          <a href={`tel:${p.replace(/\s+/g, "")}`} className="hover:text-white transition-colors whitespace-nowrap">
                            {p.trim()}
                          </a>
                          {i < arr.length - 1 && <span className="text-white/60 mx-1">,</span>}
                        </span>
                      ))}
                    </div>
                  </div>
